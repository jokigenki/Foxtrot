...
propertyChanged("_YOffset", _YOffset);
        _MovementX = asNumber(0);
propertyChanged("_MovementX", _MovementX);
        _MovementY = asNumber(0);
...
if(wrapper.enabled){
        for(item in cast(_Pipes, Array<Dynamic>))
{
            item.setX(((item.getX() - _MovementX) + (actor.getX() - _StartXPosition)));
            item.setY(((item.getY() - _MovementY) + (actor.getY() - _StartYPosition)));
}
