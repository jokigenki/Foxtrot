...
class Design_411_411_PrecalcPivotPM extends ActorScript
{          	
...
/* Params are: __Lift */
public function _customBlock_GetXPostionForLift(__Lift:Actor):Float
{
        ...
        for(item in cast(_Lifts, Array<Dynamic>))
{
            if((item == __Lift))
{
                return (_PositionsX[Std.int(_Count)] - _LiftOffsetX);
}
...
/* Params are: __Lift */
public function _customBlock_GetYPositionForLift(__Lift:Actor):Float
{
var __Self:Actor = actor;
        _Count = asNumber(0);
propertyChanged("_Count", _Count);
        for(item in cast(_Lifts, Array<Dynamic>))
{
            if((item == __Lift))
{
                return (_PositionsY[Std.int(_Count)] - _LiftOffsetY);
}
...
{
            _Angle = asNumber(Utils.RAD * ((index0 * (_Steps / 360))));
...
{
                _LiftOffsetX = asNumber(actor.getWidth()/2);
propertyChanged("_LiftOffsetX", _LiftOffsetX);
                _LiftOffsetY = asNumber(actor.getHeight()/2);
